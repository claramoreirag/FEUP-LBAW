<?php function drawPost($postID)
{

?>
    <script src='https://kit.fontawesome.com/a076d05399.js'></script>
    <link rel="stylesheet" href="../style/bootstrap.css">
    <div class="card mb-3">
        <div class="card-body ">
            <div class="row mb-2 justify-content-end">
                <div class="col-sm-3 col-md-3  " style="text-align:end">
                    <span class="badge badge-primary tag p-1">Tag</span>
                </div>
            </div>
            <div class="row">
                <div class="col-1 " style="margin-right:1rem;">
                    <img class="" src="../images/profilepic1.jpg" alt="profile pic" width="40" height="40" style="border-radius: 50%;">
                </div>
                <div class="col-10">
                    <h4 class="card-title text-primary">Low-level jets create winds of change for turbines</h4>
                </div>

            </div>

            <h6 class="card-subtitle mt-2 mb-2 text-muted">By @johndoe on 20/01/2021 20:40</h6>
            <div class="card-text">
                <div class="row">

                    <div class="col-lg-8 col-md-9 col-sm-12 mb-2">
                        <p class="mb-0">As one of the leading sources of clean and renewable energy, global wind power capacity has increased more than fivefold over the past decade, leading to larger turbines and pushing wind technology to its limits.</p>
                        <a href="../pages/fullPostPage.php" class="read-more">Read More</a>
                        <br>
                    </div>
                    <div class="col-md-4 col-xs-6 post-pic">
                        <img class="img-fluid" src="../images/news1.jpg" alt="template pic">
                    </div>
                </div>

            </div>

            <div class="row">
                <!-- <div class="col-sm-1 col-sm-2 actions"></i>
                    <div class=" share action"><i class="fas fa-share-alt"></i></div>
                </div> -->
                <div class="col-lg-2 col-md-3 col-sm-4 mt-2">
                    <div class="row justify-content-end votes">
                        <div class="col-6 upvote"><i class="fas fa-arrow-up"></i> 10 </div>
                        <div class="col-6 downvote"><i class="fas fa-arrow-down"></i> 3 </div>

                    </div>
                </div>
            </div>

        </div>

    </div>



<?php } ?>


<?php function drawAuthPost($postID)
{

?>
    <script src='https://kit.fontawesome.com/a076d05399.js'></script>
    <link rel="stylesheet" href="../style/bootstrap.css">
    <div class="card mb-3">
        <div class="card-body ">
            <div class="row mb-2 justify-content-end">
                <div class="col-sm-3 col-md-3  " style="text-align:end">
                    <span class="badge badge-primary tag p-1">Tag</span>
                </div>
            </div>
            <div class="row">
                <div class="col-1 " style="margin-right:1rem;">
                    <img class="" src="../images/profilepic1.jpg" alt="profile pic" width="40" height="40" style="border-radius: 50%;">
                </div>
                <div class="col-10">
                    <h4 class="card-title text-primary">Low-level jets create winds of change for turbines</h4>
                </div>

            </div>

            <h6 class="card-subtitle mt-2 mb-2 text-muted">By @johndoe on 20/01/2021 20:40</h6>
            <div class="card-text">
                <div class="row">

                    <div class="col-lg-8 col-md-9 col-sm-12 mb-2">
                        <p class="mb-0">As one of the leading sources of clean and renewable energy, global wind power capacity has increased more than fivefold over the past decade, leading to larger turbines and pushing wind technology to its limits.</p>
                        <a href="../pages/fullPostPage.php" class="read-more">Read More</a>
                        <br>
                    </div>
                    <div class="col-md-4 col-xs-6 post-pic">
                        <img class="img-fluid" src="../images/news1.jpg" alt="template pic">
                    </div>
                </div>

            </div>

        
            <div class="row justify-content-between text-center">
                <div class="col-md-3 col-sm-4 actions pt-3 pb-3"></i>
                    <div class="row text-secondary">
                        <div class="col-4 share action"><i class="fas fa-share-alt"></i></div>
                        <div class="col-4 save action"><i class="fas fa-bookmark"></i></div>
                        <div class="col-4 report action"><i class="fas fa-exclamation-circle"></i></div>
                    </div>
                </div>
                <div class="col-xl-2 col-md-3 col-sm-4 mt-2">
                    <div class="row justify-content-end votes">
                        <div class="col-6 upvote"><i class="fas fa-arrow-up"></i> 10 </div>
                        <div class="col-6 downvote"><i class="fas fa-arrow-down"></i> 3 </div>

                    </div>
                </div>
            </div>

        </div>

    </div>



<?php } ?>



<?php function drawOwnPost($postID)
{

?>
    <script src='https://kit.fontawesome.com/a076d05399.js'></script>
    <link rel="stylesheet" href="../style/bootstrap.css">
    <div class="card mb-3">
        <div class="card-body ">
            <div class="row mb-2 justify-content-end">
                <div class="col-sm-3 col-md-3  " style="text-align:end">
                    <span class="badge badge-primary tag p-1">Tag</span>
                </div>
            </div>
            <div class="row">
                <div class="col-1 " style="margin-right:1rem;">
                    <img class="" src="../images/profilepic1.jpg" alt="profile pic" width="40" height="40" style="border-radius: 50%;">
                </div>
                <div class="col-10">
                    <h4 class="card-title text-primary">Low-level jets create winds of change for turbines</h4>
                </div>

            </div>

            <h6 class="card-subtitle mt-2 mb-2 text-muted">By @johndoe on 20/01/2021 20:40</h6>
            <div class="card-text">
                <div class="row">

                    <div class="col-lg-8 col-md-9 col-sm-12 mb-2">
                        <p class="mb-0">As one of the leading sources of clean and renewable energy, global wind power capacity has increased more than fivefold over the past decade, leading to larger turbines and pushing wind technology to its limits.</p>
                        <a href="../pages/fullPostPage.php" class="read-more">Read More</a>
                        <br>
                    </div>
                    <div class="col-md-4 col-xs-6 post-pic">
                        <img class="img-fluid" src="../images/news1.jpg" alt="template pic">
                    </div>
                </div>

            </div>

        
            <div class="row post-interactions justify-content-between ">
                <div class="col-xs-4 col-sm-4 col-md-2 actions mt-2 text-secondary pb-3"></i>
                    <div class="row ">
                        <div class="col-md-6 col-xs-6 share action"><i class="fas fa-share-alt"></i></div>
                        <div class="col-md-6 col-xs-6 save action"><span class="fas fa-edit"></div>

                    </div>
                </div>
                <div class="col-xs-4 col-sm-4 col-md-2 mt-2 text-center" >
                    <div class="row justify-content-end votes">
                        <div class="col-6 upvote"><i class="fas fa-arrow-up"></i> 10 </div>
                        <div class="col-6 downvote"><i class="fas fa-arrow-down"></i> 3 </div>

                    </div>
                </div>
            </div>
        </div>

    </div>



<?php } ?>






<?php function drawOwnP($postID)
{

?>

    <script src='https://kit.fontawesome.com/a076d05399.js'></script>
    <link rel="stylesheet" href="../style/bootstrap.css"">

    <div class=" container-fluid" id="post">
    <div class="row">

        <div class="col-12 justify-content-end ">
            <div class="row justify-content-between post-header">
                <div class="col-xs-12 col-sm-8">
                    <div class="row ">
                        <div class=" col-sm-1" id="profile-pic"><img class="" src="../images/profilepic1.jpg" alt="template pic" width="40" height="40" style="border-radius: 50%;"></div>
                        <div class="col-10 " id="title">Low-level jets create winds of change for turbines</div>
                    </div>
                </div>
                <div class="col-sm-3 col-md-3 tag-holder ">
                    <button class="btn btn-primary tags">Tag</button>
                </div>
            </div>

            <div class="row justify-content-between post-content ">
                <div class="col-lg-8 col-md-9 col-sm-12 ">
                    <p class="post-text ">As one of the leading sources of clean and renewable energy, global wind power capacity has increased more than fivefold over the past decade, leading to larger turbines and pushing wind technology to its limits.</p>
                    <br>
                    <a href="../pages/fullPostPage.php" class="read-more">Read More</a>
                </div>
                <div class="col-3 post-pic">
                    <img class="" src="../images/news1.jpg" alt="template pic" width="170">
                </div>
            </div>
            <div class="row post-interactions justify-content-between">
                <div class="col-2 actions"></i>
                    <div class="row ">
                        <div class="col-6 share action"><i class="fas fa-share-alt"></i></div>
                        <div class="col-6 save action"><span class="fas fa-edit"></div>

                    </div>
                </div>
                <div class="col-2" style="margin-right:0rem; padding-right:0rem ;margin-top:0.5rem">
                    <div class="row justify-content-end votes">
                        <div class="col-6 upvote"><i class="fas fa-arrow-up"></i> 10 </div>
                        <div class="col-6 downvote"><i class="fas fa-arrow-down"></i> 3 </div>

                    </div>
                </div>
            </div>


        </div>

    </div>
    </div>





<?php } ?>