    ...
    <div class="row ">
        ...
        <i class="far fa-trash-alt"></i> Delete Post
            ...
            <div class="row post-header mt-5">
                ...
                <div class="col-md-5 col-lg-11"><p class="text-end">By <a  href="/admin/users/{{$post->author->id}}"> @<span>{{$post->author->username}}</span></a> on <span>{{ date('d-m-Y', strtotime($post->datetime)) }}</p></div>
                <!--Imagem do avatar-->
...
<div id="banmodal" class="modal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Are you sure you want to ban the user {{$post->author->username}} ?</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
